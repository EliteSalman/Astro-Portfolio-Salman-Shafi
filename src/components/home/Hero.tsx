...
export default function Hero({ locale = 'en-gb' }: { locale?: 'en-gb' | 'bn' }) {
  const bn = locale === 'bn';
  return (
      ...
      <div className="container mx-auto px-6 max-w-5xl">
        <div className="flex flex-col items-start space-y-6">
          ...
          <h2 className="text-xl md:text-2xl text-[#a1a1aa] font-mono">
            {bn ? 'Unix/Linux Systems & Network Engineer' : 'Unix/Linux Systems & Network Engineer'}
          </h2>
          
          <p className="text-base md:text-lg text-[#888] leading-relaxed max-w-2xl font-mono mt-2">
            {bn ? 'DIY infrastructure, শূন্য থেকে deployment এবং অত্যন্ত নিরাপদ self-hosted environment তৈরি করাই আমার কাজের মূল ফোকাস।' : 'Focus on DIY infrastructure, from-scratch deployments, and building highly secure, self-hosted environments.'}
          </p>
          ...
          <div className="flex flex-wrap gap-4 pt-4 font-mono">
            <div className="flex items-center gap-2 text-[#a1a1aa] bg-[#111] border border-[#1e1e1e] px-3 py-1.5 text-sm">
              ...
              <span>{bn ? 'বগুড়া, বাংলাদেশ' : 'Bogura, Bangladesh'}</span>
            </div>
            <div className="flex items-center gap-2 text-[#a1a1aa] bg-[#111] border border-[#1e1e1e] px-3 py-1.5 text-sm">
              <Server size={14} className="text-[#EE0000]" />
              <span>{bn ? 'Self-Hosting ও Infrastructure' : 'Self-Hosting & Infrastructure'}</span>
            </div>
          </div>
          ...
          <div className="inline-grid grid-cols-3 divide-x divide-[#1e1e1e] border border-[#1e1e1e] mt-8 font-mono bg-[#0a0a0a]">
            <div className="px-6 py-4 text-center hover:bg-[#111] transition-colors">
              ...
              <div className="text-xs text-[#555] mt-1 uppercase tracking-wider">{bn ? 'বছরের অভিজ্ঞতা' : 'Years Exp'}</div>
            </div>
            <div className="px-6 py-4 text-center hover:bg-[#111] transition-colors">
              <div className="text-2xl font-bold text-[#e5e5e5]">20+</div>
              <div className="text-xs text-[#555] mt-1 uppercase tracking-wider">{bn ? 'প্রজেক্ট' : 'Projects'}</div>
            </div>
            <div className="px-6 py-4 text-center hover:bg-[#111] transition-colors">
              <div className="text-2xl font-bold text-[#e5e5e5]">24/7</div>
              <div className="text-xs text-[#555] mt-1 uppercase tracking-wider">{bn ? 'সাপোর্ট' : 'Support'}</div>
            </div>
          </div>
          ...
          <div className="pt-8 font-mono">
            ...
            >
              {bn ? 'যোগাযোগ শুরু করুন' : 'INITIALISE_CONTACT'}
