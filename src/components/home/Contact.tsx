...
export default function Contact({ locale = 'en-gb' }: { locale?: 'en-gb' | 'bn' }) {
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    if (!turnstileToken || turnstileToken.length === 0) {
      setFormStatus("error");
      setStatusMessage(
        "Security verification failed. Please complete the CAPTCHA.",
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormStatus("error");
        setStatusMessage(
          result.error || "Transmission failed. Connection refused.",
        );
        if ((window as any).turnstile && turnstileWidgetId.current !== null) {
          (window as any).turnstile.reset(turnstileWidgetId.current);
        }
      ...
      setFormStatus("error");
      setStatusMessage(
        "Network unreachable. Please verify routing and try again.",
      );
      if ((window as any).turnstile && turnstileWidgetId.current !== null) {
        (window as any).turnstile.reset(turnstileWidgetId.current);
      }
  ...
  return (
    <section
      id="contact"
      className="py-24 bg-black border-b border-[#1e1e1e] font-mono scroll-mt-24"
    >
      <div className="container mx-auto px-6 max-w-5xl">
        <div className="text-sm font-medium text-[#a1a1aa] mb-12 flex items-center whitespace-nowrap overflow-x-auto no-scrollbar">
          ...
          <span className="text-white ml-2">
            $ /usr/local/bin/send-message --to=
            <a
              href="mailto:hello@salmanshafi.net"
              className="hover:text-[#EE0000] underline underline-offset-4"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          >
            <form
              ...
              className="flex flex-col space-y-6 bg-[#050505] border border-[#1e1e1e] p-6 md:p-8"
            >
              {statusMessage && (
                <motion.div
                  variants={item}
                  className={`p-3 text-sm border ${formStatus === "success" ? "border-green-900 bg-green-900/10 text-green-500" : formStatus === "error" ? "border-[#EE0000]/30 bg-[#EE0000]/10 text-[#EE0000]" : "border-[#1e1e1e] text-[#a1a1aa]"}`}
                >
                  {formStatus === "success"
                    ? "[OK] "
                    : formStatus === "error"
              ...
              >
                <label
                  htmlFor="name"
                  className="text-[#555] mb-2 text-sm flex items-center"
                >
              ...
              <motion.div
                variants={item}
                className="cf-turnstile flex flex-col space-y-2"
              >
                {!turnstileLoaded && (
                  <span className="text-[#555] text-xs">
                    {runtimeSiteKey
                      ? "Loading verification module..."
                      : "Fetching security keys..."}
                  </span>
                )}
                {runtimeSiteKey && (
            ...
            >
              ...
              <div className="space-y-4 text-sm">
                <div className="grid grid-cols-12 gap-2">
                  <div className="col-span-12 sm:col-span-3 text-[#555]">
                    email_uri
                  </div>
                  <div className="col-span-12 sm:col-span-9 text-[#e5e5e5] break-words">
                    = &quot;
                    <a
                      href="mailto:hello@salmanshafi.net"
                      className="hover:text-[#EE0000] underline underline-offset-4"
